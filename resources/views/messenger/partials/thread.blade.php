...
<li class="notification {{ $class }}">
    <div class="media">
      ...
      <div class="media-body">
        <strong class="notification-title res-text-9"><a href="{{ route('messages.show', $thread->id) }}">{{ $thread->participantsString(Auth::id()) }} {{ $unreadCount }} </a></strong>
        <p class="notification-desc res-text-9"><a href = "{{ route('messages.show', $thread->id) }}" class = "text-muted">{{ $thread->latestMessage->body }}</a></p>

        <div class="notification-meta">
          <span class="timestamp float-left res-text-9">{{ Carbon\Carbon::parse($thread->created_at)->format('M j Y @ G:i A') }}</span>
        </div>
      </div>
    </div>
</li>